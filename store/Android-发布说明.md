...
Android 0.2.0（versionCode 2），支持 Android 8.0（API 26）及以上；编译与目标版本为 Android 16（API 36）。不含广告或统计 SDK。新增网络权限用于可选 WebDAV 同步，默认关闭；后台同步由 JobScheduler 调度，可能因省电延迟。详见 [同步使用说明](../同步使用说明.md)。真实坚果云账号与真实手机尚未验收。
