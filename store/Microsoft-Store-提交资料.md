...
待办默认保存在本机，无需注册账户，无广告、分析或跟踪 SDK。Windows 1.3.0 新增可选 HTTPS WebDAV 同步，需用户自行连接网盘；待办内容会上传到指定服务，云端文件未端到端加密。当前为同步预览，正式商店提交前仍需真实服务验收。
...
输出：`dist/QingJian-1.3.0.0-x64-store.msix`。商店签名在通过审核并发布时处理。不要把本地测试证书或私钥上传仓库。
